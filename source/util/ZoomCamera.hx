...
class ZoomCamera extends FlxCamera
{
	...
	public function new(scale:Float = 1, ?Zoom:Float = 0)
	{
		Zoom = Zoom == 0 ? 1/scale : Zoom;
		screenScale = scale;
		super(0,0,Std.int(FlxG.width*screenScale), Std.int(FlxG.height*screenScale), 1/screenScale);
	...
	public override function update():Void
	{
		...
		// Update camera zoom	
		if(Math.abs(zoom - targetZoom) < 0.0002) {
			zoom = targetZoom;
		}
		else {
			zoom += (targetZoom - zoom) / 10 * (FlxG.elapsed) * _zoomSpeed;
		}
